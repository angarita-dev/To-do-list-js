import { itemCheck, projectSelect, exitProjectEditMode, enterProjectEditMode } from './togglers.js';
...
function saveToDoItems(){
  ...
  let toDoList = toDoItems.map( toDoItem => {
    return {
      priority: toDoItem.firstElementChild.classList[0],
...
export default () => { loadToDoChecker(); loadClickableProjects();
  loadEditIcons();
  loadPrioritySelector();
  loadAddProject();
  loadStorageProjects();
